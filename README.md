...
| √s | 13.6 TeV (Run3), 13.0 TeV (Run2) |
| Luminosity | 17.794 fb⁻¹ (2023), 59.83 fb⁻¹ (2018) |
...
**Current project status.** The most recent completed training is `bdt_v4_sr.json` (Path 1), which applies a signal-region pre-selection (m₄ℓ ∈ [105, 140] GeV) before training and retains the full 23-feature input set, including m₄ℓ. Its in-repository metrics (AUC, |r|, KS) meet the Path 1 targets, but downstream application of v4_sr in the SKNanoAnalyzer pipeline produced an unsatisfactory result. The project is currently iterating on **v5**, which retargets the analysis to Run2 / 2018 (L = 59.83 fb⁻¹, √s = 13 TeV) using the same Path 1 procedure on Run2 MC inputs. There is no production-default model at this time. See the [Iteration History](#iteration-history) section for the full v1 → v2 → v3 → Path 1 → v5 progression and the [Open issue](#open-issue-and-next-iteration--moving-to-2018) subsection for the v5 plan.
...
| **v2** | Mass-related features removed (17 vars) | 0.935 | 0.55 | Partial decorrelation; superseded by v4_sr |
| **v3** | Per-class planing of m₄ℓ distribution | 0.81 | **0.73** | Decorrelation worsened; failure analyzed |
| **Path 1 (v4_sr)** | SR-restricted training (m₄ℓ ∈ [105, 140] GeV), 13.6 TeV / 2023 MC | 0.925 | **0.094** ✓ | In-repo targets met; downstream application in SKNanoAnalyzer unsatisfactory |
| **v5** *(in progress)* | Path 1 retargeted to Run2 / 2018, √s = 13 TeV, L = 59.83 fb⁻¹ | — | — | Active iteration — see [Open issue](#open-issue-and-next-iteration--moving-to-2018) |
...
v2 was retained as the project default through the v3 planing iteration because the AUC trade-off was considered acceptable for the learning-project objectives, and because v3 (planing) was subsequently shown to perform worse. It was superseded by v4_sr (Path 1), which met the |r| < 0.1 target with classification power that was comparable in-window. v2 is now kept only as the reference "mass-removed, wide-window" baseline.
...
### Path 1 — Signal-region-restricted training (v4_sr)
...
`bdt_v4_sr.json` met the Path 1 in-repository targets: classification performance (AUC = 0.925 in-window) and score-mass independence (|r| = 0.094). When applied in the SKNanoAnalyzer downstream pipeline, however, its observed performance was unsatisfactory (see the next subsection for the working hypothesis and the v5 plan). The Path 1 *procedure* — SR pre-selection + full 23-feature input + in-region training — is retained as the project workflow; the v4_sr *model file itself* is being superseded by the in-progress v5 iteration. v2 and v3 are kept in the repository as documented historical iterations.
...
The training input list is defined in `src/features.py::FEATURES` (23 variables; restored to the full set from Path 1 / v4_sr onward, and used unchanged for the in-progress v5):
...
```bash
# Defaults to data/models/bdt_v4_sr.json → bdt_v4_sr.onnx (most recent completed model)
